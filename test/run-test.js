...
const site = http.createServer((q, r) => {
  ...
  if (q.url === '/v1/messages') { let b = ''; q.on('data', c => b += c); return q.on('end', () => { calls++; const img = JSON.parse(b).messages[0].content[0]; const ok = img.type === 'image' && img.source.data.length > 1000;
  ...
  const run = extra => new Promise(res => { const c = spawn('node', [path.join(__dirname, '..', 'src', 'run.js'), ...extra], { env }); let stdout = ''; c.stdout.on('data', d => stdout += d); c.stderr.on('data', d => stdout += d); c.on('close', status => res({ status, stdout })); });
  ...
  fs.writeFileSync(out, keep); fs.rmSync(path.join(__dirname, '..', 'data', 'history'), { recursive: true, force: true });
